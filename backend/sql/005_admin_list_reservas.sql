    ...
    'items', coalesce((
      ...
      from (
        select
          ...
          c.telefono,
          c.correo,
          rc.carro_id::text as carro_id,
          cr.marca as car_marca,
          cr.modelo as car_modelo,
          rcs.casa_id::text as casa_id,
          rcs.personas,
          ca.nombre as casa_nombre
        from public.reservas r
        join public.clientes c on c.id = r.cliente_id
        left join public.reserva_carros rc on rc.reserva_id = r.id
        left join public.carros cr on cr.id = rc.carro_id
        left join public.reserva_casas rcs on rcs.reserva_id = r.id
        left join public.casas ca on ca.id = rcs.casa_id
